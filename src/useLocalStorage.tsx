...
export function useLocalStorage<T>(
  ...
  const setValue = useCallback((value: T | ((prevValue: T) => T)) => {
    try {
      ...
      const result = localStorageManager.setItem(key, valueToStore, optionsRef.current);
  ...
  useEffect(() => {
    ...
    const handleCustomStorageChange = (e: CustomEvent<LocalStorageEventDetail<T>>) => {
      if (e.detail.key === key) {
  ...
  }, [key, isBrowser]); // Removed refreshValue from dependencies

  // Auto-refresh when key changes
  useEffect(() => {
    refreshValue();
  }, [key]); // Removed refreshValue from dependencies
    ...
    const interval = setInterval(() => {
      ...
      if (expired) {
        try {
          ...
          setError(err instanceof Error ? err : new Error('Failed to remove expired item'));
        }
      }
    }, Math.min(options.ttl / 10, 60000)); // Check every 1/10 of TTL or max 1 minute
